...
function drawLtTrend(){
  ...
  document.getElementById('ltTrendLegend').innerHTML='';
  const sts=LEAD.stations.filter(s=>LEAD.months.some(m=>ltGet(m,s.id)));
  // grade de mini gráficos de linha (um por estação), mesma escala p/ comparar
  let gmax=0; sts.forEach(s=>LEAD.months.forEach(m=>{const r=ltGet(m,s.id);if(r&&r.lt_d!=null&&r.lt_d>gmax)gmax=r.lt_d;}));
  const top=Math.ceil(gmax)||1;
  const W=300,H=110,m={t:12,r:12,b:22,l:30};
  const x=i=>N<=1?W/2:m.l+i*((W-m.l-m.r)/(N-1));
  const y=v=>m.t+(top-v)/top*(H-m.t-m.b);
  const grid=cssv('--grid'),muted=cssv('--muted');
  let html='<div class="smgrid" style="grid-template-columns:repeat(auto-fill,minmax(280px,1fr))">';
  sts.forEach((s,si)=>{
    const col=cssv(SERIES[si%8]);
    const vals=LEAD.months.map(mm=>{const r=ltGet(mm,s.id);return r?r.lt_d:null;});
    const present=vals.filter(v=>v!=null);
    const avg=present.length?present.reduce((a,b)=>a+b,0)/present.length:null;
    const last=[...vals].reverse().find(v=>v!=null);
    let sv=`<svg viewBox="0 0 ${W} ${H}" width="100%" height="${H}" preserveAspectRatio="none" role="img">`;
    for(let t=0;t<=2;t++){const val=top*t/2,yy=y(val);sv+=`<line x1="${m.l}" y1="${yy}" x2="${W-m.r}" y2="${yy}" stroke="${grid}"/>`;sv+=`<text x="${m.l-5}" y="${yy+3}" text-anchor="end" font-size="9" fill="${muted}">${val.toFixed(0)}</text>`;}
    LEAD.months.forEach((mm,i)=>{sv+=`<text x="${x(i).toFixed(1)}" y="${H-6}" text-anchor="middle" font-size="9" fill="${muted}">${LEAD.mlabels[mm]}</text>`;});
    let d='',st=false; vals.forEach((v,i)=>{if(v==null){st=false;return;}d+=(st?'L':'M')+x(i).toFixed(1)+' '+y(v).toFixed(1)+' ';st=true;});
    sv+=`<path d="${d}" fill="none" stroke="${col}" stroke-width="2.4" stroke-linejoin="round" stroke-linecap="round"/>`;
    vals.forEach((v,i)=>{if(v!=null){sv+=`<circle cx="${x(i).toFixed(1)}" cy="${y(v).toFixed(1)}" r="3.2" fill="${col}"/>`;
      sv+=`<text x="${x(i).toFixed(1)}" y="${(y(v)-6).toFixed(1)}" text-anchor="middle" font-size="9.5" font-weight="600" fill="${cssv('--ink')}">${v.toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})}</text>`;}});
    sv+='</svg>';
    html+=`<div class="smcard"><div class="nm"><i style="background:${col}"></i>${s.name}</div>`+
      `<div class="st">média <b>${avg!=null?avg.toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})+' d':'—'}</b> · último ${last!=null?last.toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})+' d':'—'}</div>${sv}</div>`;
  });
  html+='</div>'; wrap.innerHTML=html;
}
